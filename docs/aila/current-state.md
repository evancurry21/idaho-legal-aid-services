...
| Promptfoo gate behavior | Evaluation / release gating | `confirmed with deploy-bound split` | Synthetic assistant eval gate and policy enforcement | GitHub Actions + operator-run local/DDEV/Pantheon targeting | TOVR-05 proves PR/helper branches use real hosted advisory checks, protected pushes use real hosted blocking checks, and synced `origin/master` deploys are blocked by a local DDEV exact-code gate using `promptfooconfig.deploy.yaml` | `high` | Fresh post-change workflow-history proof for the next protected-branch push run and next clean `origin/master` deploy push | [^CLAIM-188][^CLAIM-200][^CLAIM-211] |
...
TOVR-05 (2026-03-16) hardened release gating so the live-safe deploy profile is the shared promptfoo suite across hosted and deploy-bound paths: PR/helper-branch GitHub checks now run real hosted advisory evals, protected-branch pushes run real hosted blocking evals, and synced `origin/master` deploys now block on a local DDEV exact-code gate instead of treating the hosted GitHub check as deploy proof.[^CLAIM-200][^CLAIM-211]
...
| Promptfoo + quality gate harness | The repo now has an active first-party `Quality Gate` workflow, and TOVR-05 converts the previously mixed advisory/simulated posture into an explicit split: PR/helper branches run real hosted promptfoo evals in advisory mode with `promptfooconfig.deploy.yaml`, protected-branch pushes run the same deploy profile in blocking hosted mode, and synced `origin/master` deploys run the local DDEV exact-code deploy-bound gate before Pantheon push. The hosted GitHub check remains useful hosted-environment evidence, but it is no longer treated as deploy proof for `origin/master`.[^CLAIM-086][^CLAIM-188][^CLAIM-200][^CLAIM-211] |
...
| Promptfoo deploy-bound gate fidelity | `repo remediated / post-change hosted history pending` | Prior status was proven from workflow history: PR run `23165709608` logged a real promptfoo eval in advisory mode, while protected-branch push run `23166428930` logged simulated config-parity mode in blocking status. Post-change repo wiring now switches PR/helper branches to real hosted deploy-profile advisory runs, protected-branch pushes to real hosted deploy-profile blocking runs, and synced `origin/master` pushes to a local DDEV exact-code blocking gate. | Capture the next post-change protected-branch workflow run and next clean `origin/master` deploy push to extend workflow-history proof beyond repo/script inspection. Hosted GitHub proof remains intentionally scoped to hosted checks, while deploy proof now comes from the local DDEV gate. | [^CLAIM-200][^CLAIM-211] |
