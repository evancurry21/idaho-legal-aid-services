...
Aila is a Drupal custom-module assistant exposed as `/assistant` and `/assistant/api/*`. The backend pipeline is deterministic first (flood controls, validation, a shared pre-routing decision engine over safety/out-of-scope/policy checks, intent routing, retrieval), with bounded request-time LLM classification on Cohere behind runtime gates, Voyage for embeddings/reranking, and residual Gemini config retained only where Search API AI still proves a vector/chat dependency. Observability hooks still cover Drupal logs, analytics tables, internal health/metrics surfaces, Langfuse queue export, effective Sentry browser/PHP wiring, dormant New Relic paths, and repo-owned promptfoo/GitHub Actions quality gating.[^CLAIM-010][^CLAIM-011][^CLAIM-033][^CLAIM-038][^CLAIM-045][^CLAIM-069][^CLAIM-188][^CLAIM-189][^CLAIM-190][^CLAIM-191][^CLAIM-193]

### 2026-04-15 Cohere-first transition note

1. Assistant request-time generation/classification is now Cohere-first and runtime-toggle controlled through `ILAS_COHERE_API_KEY` plus `ILAS_LLM_ENABLED`.
2. Greeting variation remains retired; only ambiguous-intent classification is in scope for the request-time LLM path.
3. Voyage remains the embeddings and reranking provider.
4. Gemini remains a residual vector/Search API concern only where repo/runtime proof still requires it.
5. Live vector search is now runtime-toggle controlled through `ILAS_VECTOR_SEARCH_ENABLED` while stored Drupal config remains `false` and the live admin form stays disabled.

### 2026-04-21 live Pinecone runtime-toggle contract refresh

1. `settings.php` no longer hard-forces `vector_search.enabled=false` on `live`; the existing runtime-only toggle path now honors `live` alongside `local`, `dev`, and `test`.
2. Live admin saves still cannot enable vector search through Drupal config. The form stays disabled on `live`, validation still rejects the checkbox, and submit handling still coerces stored config back to `false`.
3. `ilas:runtime-truth` and `ilas:vector-status` are the canonical verification surfaces for live enablement and rollback.
4. TOVR-13 remains the historical blocked live-readiness baseline, and TOVR-17 supersedes it for the current live Pinecone contract.
...
| LLM enhancement | Disabled in exported active config (`llm.enabled: false`) | Runtime-enableable through `ILAS_LLM_ENABLED` plus `ILAS_COHERE_API_KEY`; request-time provider is Cohere when runtime-ready | Greeting variation remains retired; Gemini is no longer the assistant request-time provider | [^CLAIM-069][^CLAIM-094][^CLAIM-099][^CLAIM-119] |
...
| Langfuse tracing/export | Install/export defaults remain disabled; effective export is now live-only by default and controlled by `ILAS_LANGFUSE_ENABLED` for explicit diagnostics or kill-switch use | `dev`/`test` remain disabled even when Langfuse secrets exist; `live` auto-enables when credentials exist unless `ILAS_LANGFUSE_ENABLED=0` | Non-live exports require `ILAS_LANGFUSE_ENABLED=1`; queue age thresholds are 7200s to tolerate normal Pantheon cron spacing without avoidable stale loss. | [^CLAIM-079][^CLAIM-082][^CLAIM-189][^CLAIM-190][^CLAIM-209][^CLAIM-210][^CLAIM-248][^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-262] |
...
| Langfuse runtime export | Observability | `confirmed partial` | Queue-backed trace/span/event export | `local`, `dev`, `test`, `live` | Safe runtime booleans still show enabled effective config everywhere; TOVR-04 proves fresh local direct and queued traces in Langfuse API with privacy-safe payload summaries; TOVR-13 adds vector/lexical retrieval metadata to repo-side trace payloads; AFRP-03 then adds lookup/counter surfaces locally, proves fresh local direct/queued/request-path traces, proves local queue-loss accounting, and reruns current hosted `dev`/`test`/`live` direct+queued probes plus one hosted `live` request-path trace account-side | `high` | Deploy AFRP-03's new status/lookup/counter surfaces to Pantheon and rerun hosted queue-loss drills from deployed code if operator-visible parity is required | [^CLAIM-189][^CLAIM-190][^CLAIM-210][^CLAIM-237][^CLAIM-248][^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-262] |
...
| Search API vector indexes | Retrieval infrastructure | `lexical-first retrieval enabled under runtime controls` | Provide optional Pinecone-backed FAQ/resource vector indexes | Repo config remains present; `local` / `dev` / `test` already runtime-enable vector supplementation, and the TOVR-17 repo contract now makes `live` runtime-toggle controlled as well while stored config stays `false` until the hosted secret is flipped | TOVR-12 and TOVR-13 preserve the historical non-live/live gate evidence. TOVR-17 updates the current repo contract so live eligibility is now governed by deployed code + runtime secret verification rather than a hard-coded live `false` override. | `high` | Hosted live flip and rollback verification are still deployment-bound: deploy code, set `ILAS_VECTOR_SEARCH_ENABLED=1`, clear caches, and capture runtime-truth/vector-status/smoke evidence. | [^CLAIM-066][^CLAIM-067][^CLAIM-216][^CLAIM-221][^CLAIM-222][^CLAIM-223][^CLAIM-224][^CLAIM-225][^CLAIM-226][^CLAIM-227][^CLAIM-228][^CLAIM-229][^CLAIM-230][^CLAIM-231][^CLAIM-232][^CLAIM-233][^CLAIM-235][^CLAIM-238][^CLAIM-246][^CLAIM-248][^CLAIM-250] |
...
TOVR-09 (2026-03-17) established the then-current Pinecone baseline before the later non-live runtime enablement work: Pinecone key presence was `true`, hosted `dev`/`test`/`live` already had lexical and vector indexes enabled and queryable, and `local` still differed materially because of missing lexical indexes, pending updates `10008`/`10009`/`10010`, and blocked vector queries caused by stale active config around the Gemini key path. TOVR-12, TOVR-16, and AFRP-02 supersede the old `vector_search.enabled=false everywhere` assumption.[^CLAIM-216][^CLAIM-217][^CLAIM-218][^CLAIM-220]
...
TOVR-13 (2026-03-17) executes the historical production live gate review. Current hosted `live` no longer had basic secret/index ambiguity at that point: runtime truth showed Pinecone, Langfuse, and Sentry effective on release `live_148`, both vector indexes were fully indexed and healthy, fresh Sentry and Langfuse probes still landed, and `live` LLM remained off. The final verdict was `Blocked with explicit evidence` because `live` vector was still hard-forced off, the latest `master` Quality Gate run `23218168501` was red, `diagnostics_token_present=false` left positive machine-auth monitoring unproven, prompts 2 / 3 still lacked accepted vector-quality proof, and embeddings-side timeout governance remained shared/global. TOVR-17 later supersedes that live hard-force with a runtime-toggle contract while preserving TOVR-13 as the historical blocked baseline.[^CLAIM-233][^CLAIM-234][^CLAIM-235][^CLAIM-236][^CLAIM-237][^CLAIM-238]
TOVR-14 (2026-03-17) reduces the enabled AI/provider module footprint to the minimum proven vector stack: `ai`, `ai_search`, `ai_vdb_provider_pinecone`, `gemini_provider`, and `key` remain enabled because the current `pinecone_vector_faq` and `pinecone_vector_resources` Search API servers still depend on Search API AI plus Gemini embeddings/chat and Pinecone key wiring, while dormant `ai_provider_google_vertex`, `ai_seo`, and `metatag_ai` were uninstalled and their module-owned config removed. Local post-change runtime truth, Search API inventory, and assistant smoke remained stable. The 2026-04-15 Cohere-first transition later removed the custom assistant Vertex runtime-secret loading from `settings.php`; residual provider surface is now limited to the Search API / vector Gemini footprint plus any unused contrib packages still awaiting a separate removal proof, and hosted post-change verification remains `Unverified` until a deploy occurs.[^CLAIM-239][^CLAIM-240][^CLAIM-241][^CLAIM-242]
...
TOVR-17 (2026-04-21) refreshes the live Pinecone repo contract. `settings.php` no longer hard-forces live vector retrieval off, `ILAS_VECTOR_SEARCH_ENABLED` now governs effective live vector enablement just as it already does on non-live, the live admin/config path still keeps stored `vector_search.enabled=false`, and vector-index hygiene refresh now rewrites stale expected metadata from current config instead of preserving legacy state from existing snapshots. Hosted live enablement remains deployment-bound: the new contract still requires code on `live`, `ILAS_VECTOR_SEARCH_ENABLED=1`, cache clear, `ilas:runtime-truth`, `ilas:vector-status`, and live smoke/rollback proof before runtime truth should flip effective `true`.
AFRP-03 (2026-03-19) rechecks Langfuse trust from current runtime evidence instead of inherited TOVR notes. The repo now exposes a sanitized `ilas:langfuse-lookup` command plus explicit queue/export outcome counters on `ilas:langfuse-status`, fresh local direct/queued/request traces are proven account-side, local max-depth and stale-item loss drills are recorded explicitly, and fresh hosted `dev`/`test`/`live` direct+queued probes plus one hosted `live` request-path trace are all proven account-side on current releases. The remaining Langfuse gap is no longer trace existence on current releases; it is deployment of the AFRP-03 lookup/counter surfaces to Pantheon and the still-awkward Langfuse top-level `environment=default` view, which currently requires reading the minimized metadata fields for the true runtime environment.[^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-262]
AFRP-17 (2026-03-25) closes the repo-side Langfuse queue-loss actionability gap without widening the payload boundary or adding a dead-letter queue. `QueueHealthMonitor` now classifies outcomes, tracks aggregate queue-item/event totals, exposes `action_required` plus alertable/informational loss totals, and adds explicit terminate-side enqueue-failure accounting. `SloAlertService` now warns on new alertable queue-loss outcomes while `/assistant/api/health` and `/assistant/api/metrics` expose the same structured summary under `queue.export` without turning historical counters into sticky `503` health failures. Local clean-room validation proved a stale-item drill increments `discard_stale` counters and totals (`queue_items=1`, `event_count=1`) and flips `action_required=true`; the remaining operational gap is still hosted deployment/parity for the command surfaces, while exact local `ilas:langfuse-status` / `ilas:langfuse-probe --diagnose` reruns were blocked by an unrelated `runtime_diagnostics_commands` Drush instantiation issue in the current worktree.[^CLAIM-276][^CLAIM-277][^CLAIM-278]

Primary request flow diagram: `docs/aila/system-map.mmd`.[^CLAIM-038][^CLAIM-043][^CLAIM-045]

## 2) System map

- Diagram A (`flowchart LR`) in `docs/aila/system-map.mmd` captures browser/widget -> Drupal -> integrations (Search API, Pinecone, Cohere request-time classification, residual Gemini vector/Search API provider wiring, Langfuse, Sentry, GA4, Promptfoo).[^CLAIM-011][^CLAIM-060][^CLAIM-066][^CLAIM-073][^CLAIM-079][^CLAIM-083][^CLAIM-086]
...
| Enabled AI/provider footprint | Drupal modules + synced config | `config/core.extension.yml`; `config/search_api.server.pinecone_vector_faq.yml`; `config/search_api.server.pinecone_vector_resources.yml`; `config/key.key.pinecone_api_key.yml` | Current minimal enabled set is `ai`, `ai_search`, `ai_vdb_provider_pinecone`, `gemini_provider`, and `key`; dormant `ai_provider_google_vertex`, `ai_seo`, and `metatag_ai` were removed in TOVR-14 while Composer packages remain installed | [^CLAIM-239][^CLAIM-241] |
| Search/vector config entities | Drupal config export | `config/search_api.server.pinecone_vector_faq.yml`; `config/search_api.server.pinecone_vector_resources.yml`; vector index configs | Two Search API Pinecone servers map to one shared Pinecone index (`ilas-assistant`) with per-index namespaces/collections for FAQ and resource vectors | [^CLAIM-066][^CLAIM-067] |
...
| `ilas_site_assistant.llm_enhancer` | `Drupal\ilas_site_assistant\Service\LlmEnhancer` | Cohere-first request-time classification orchestration with cache/cost/control guards | `'@config.factory', '@http_client', '@logger.factory', '@ilas_site_assistant.policy_filter', '@cache.ilas_site_assistant', '@ilas_site_assistant.llm_circuit_breaker', '@ilas_site_assistant.llm_rate_limiter', '@ilas_site_assistant.cost_control_policy', '@ilas_site_assistant.environment_detector', '@ilas_site_assistant.cohere_llm_transport'` | [^CLAIM-020] |
| `ilas_site_assistant.fallback_gate` | `Drupal\ilas_site_assistant\Service\FallbackGate` | Rule-based gate for bounded request-time LLM fallback | `'@config.factory', '@ilas_site_assistant.environment_detector', '@ilas_site_assistant.llm_enhancer'` | [^CLAIM-020] |
...
| `ilas_site_assistant.langfuse_terminate_subscriber` | `Drupal\ilas_site_assistant\EventSubscriber\LangfuseTerminateSubscriber` | Queues Langfuse payloads on response/terminate with a one-shot flush guard | `'@ilas_site_assistant.langfuse_tracer', '@queue', '@config.factory', '@logger.channel.ilas_site_assistant'` | [^CLAIM-020] |
...
| `view.assistant_governance_conversations.page_all` | `/admin/reports/ilas-assistant/governance/conversations` | `ANY` | `view assistant governance conversations`; No CSRF header requirement | Views display over canonical governance storage | Conversation log list | [^CLAIM-011] |
| `ilas_site_assistant_governance.conversation_detail` | `/admin/reports/ilas-assistant/governance/conversations/{conversation_id}` | `ANY` | `view assistant governance conversations`; No CSRF header requirement | `\Drupal\ilas_site_assistant_governance\Controller\GovernanceConversationController::detail` | Conversation log detail | [^CLAIM-011] |
...
| Event subscriber | `LangfuseTerminateSubscriber` | Queues Langfuse payload on `kernel.response` or `kernel.terminate` with depth guard and one-shot flush protection | [^CLAIM-081] |
...
| Deterministic dependency degrade contract (Phase 1 Objective #2) | Dependency failure handling is deterministic by response class: missing required retrieval dependencies now fail closed or degrade explicitly (`explicit_degraded` / endpoint-shaped unavailable) instead of silently switching to legacy/entity-query results, vector-only loss preserves lexical retrieval, and uncaught controller failures always return controlled `500 internal_error` (no partial mixed-mode response).[^CLAIM-048][^CLAIM-063][^CLAIM-065] |
...
| Integration failure contract formalization (IMP-REL-01) | All failure paths are documented as a consolidated matrix with deterministic fallback classes (`explicit_degraded`, `legacy_fallback`, `lexical_preserved`, `original_preserved`, `internal_error`) and cross-cutting request_id presence. Controller catch-all, observability isolation, and correlation ID header consistency are contract-tested.[^CLAIM-048] |
...
| Response contract expansion (`P2-DEL-01`) | All 200-response paths now include `confidence` (float 0-1, from FallbackGate or 1.0 for deterministic exits), `citations[]` (formalized from ResponseGrounder `sources`), and `decision_reason` (human-readable string from reason codes or path-specific defaults). Error responses (4xx/5xx) are excluded. Assembled by `assembleContractFields()` across the current safety, repeated-message, OOS, policy, normal, and retrieval-unavailable degraded response branches.[^CLAIM-134] |
...
| Search API usage | FAQ path uses `faq_accordion`; resources prefer `assistant_resources` with fallback to `content` index. AFRP-01 reruns on 2026-03-18 confirmed `faq_accordion` is configured and enabled in `local`, `dev`, `test`, and `live`. The English `eviction` FAQ probe is now same-language on `local`, remained same-language on sampled `dev`/`test`, and still leaks translated parent URLs on deployed `live` until this remediation ships there.[^CLAIM-060][^CLAIM-064][^CLAIM-217][^CLAIM-221][^CLAIM-252][^CLAIM-255] |
...
| Pinecone details | Search API AI server `pinecone_vector_faq` targets Pinecone index `ilas-assistant` with collection `faq_accordion_vector`, while `pinecone_vector_resources` targets the same Pinecone index with collection `assistant_resources_vector`; both use cosine similarity, Gemini embeddings, and 3072 dimensions.[^CLAIM-066] |
| Vector indexes | Vector indexes exist for FAQ paragraphs and resource nodes on distinct Search API servers (`pinecone_vector_faq`, `pinecone_vector_resources`) and corresponding namespaces/collections (`faq_accordion_vector`, `assistant_resources_vector`). TOVR-12 restored successful local direct vector probes, revalidated hosted non-live provenance on `dev` / `test`, and kept lexical-first control paths stable after runtime enablement. Only the first fixed prompt currently exposes clear mixed lexical/vector contribution in answer mode, so the remaining gap is quality proof rather than raw index availability.[^CLAIM-067][^CLAIM-217][^CLAIM-221][^CLAIM-223][^CLAIM-229][^CLAIM-230][^CLAIM-232] |
| Vector index hygiene + metadata standards + refresh monitoring (`P2-DEL-03`) | Managed vector indexes (`faq_accordion_vector`, `assistant_resources_vector`) now run policy-versioned hygiene snapshots with incremental-only refresh cadence checks, metadata drift detection (`server_id`/`metric`/`dimensions`), backlog counters, semantic queryability probes, overdue tracking, and operator-visible resume/rebuild stop reasons. TOVR-10 still shows mixed freshness across Pantheon (`dev` overdue, `test` due, `live` healthy), but the hygiene service now distinguishes metadata/config health, backlog/indexing health, and bounded live queryability proof instead of treating citation-free metadata checks as sufficient proof of vector operability.[^CLAIM-066][^CLAIM-067][^CLAIM-121][^CLAIM-136][^CLAIM-222][^CLAIM-223] |
| Toggles/flags | Vector supplement behavior is gated by config (`vector_search.*`) with admin form persistence and schema/export parity enforcement. Exported config remains `vector_search.enabled=false`, while the current runtime-truth baseline on 2026-03-18 shows effective `true` in `local`, `dev`, and `test`, with `live` still forced to `false`.[^CLAIM-061][^CLAIM-095][^CLAIM-096][^CLAIM-124][^CLAIM-216][^CLAIM-218] |
...
paragraph-level `search_api_language=en` was not sufficient to keep English FAQ
results in-language, because translated parent nodes on `local` and deployed
`live` still resolved to `/es`, `/sw`, and `/nl` parent URLs for the same
English `eviction` query. `FaqIndex` now applies one shared same-language gate
to lexical, vector, legacy, `getById()`, and category branches using resolved
parent URL plus parent-node language where available, and it fails closed when
that language cannot be proved. New pure-unit regression coverage now fails if
mixed-language FAQ URLs or category labels leak into an English request.[^CLAIM-252][^CLAIM-253][^CLAIM-254][^CLAIM-255]

AFRP-14 addendum (2026-03-25): extends language isolation to all ResourceFinder
legacy fallback paths. Three methods — `findByTypeLegacy()`, `findByTopic()`, and
`findByServiceArea()` — used `loadLegacyResourceCandidates()` which queried nodes
without a `langcode` condition and returned results without URL-based language
validation. Fix applies defense-in-depth: query-level `langcode` condition plus
`filterResourcesByCurrentLanguage()` post-filter in each caller. This also closes
a subtle injection vector where `findByTopic()` (called for sparse-result topic
boost from the Search API lexical path) could inject unfiltered foreign-language
resources. All 10 retrieval branches across FaqIndex and ResourceFinder now have
verified language isolation.[^CLAIM-269][^CLAIM-270][^CLAIM-271][^CLAIM-272]

### E) Generation / LLM integration

| Spec item | Current state |
|---|---|
| What it is | `LlmEnhancer` now performs bounded request-time intent classification for ambiguous messages only; greeting variation and free-form summary generation remain retired.[^CLAIM-045][^CLAIM-069] |
| Provider wiring | The custom assistant request-time path is Cohere-only through `CohereLlmTransport`; residual Gemini wiring is kept separate and only where Search API/vector behavior still proves it is required.[^CLAIM-073][^CLAIM-074] |
| Prompt construction | Prompt is a strict canonical-intent JSON classification contract with no-legal-advice and no-freeform-output constraints.[^CLAIM-070] |
| Sampling params | Uses bounded `max_tokens`, low `temperature`, Cohere `safety_mode`, request timeout, and connect timeout for structured JSON responses.[^CLAIM-072] |
| Guardrails | Circuit breaker + global rate limiter + retry/backoff + request timeout + deterministic post-classification re-entry into the existing routing pipeline.[^CLAIM-075][^CLAIM-077][^CLAIM-078] |
| State/caching | Policy-versioned cache key with configurable TTL for request-time classification responses.[^CLAIM-076] |
| Tool/function calling | No explicit tool/function-calling fields are present; the request-time path asks Cohere for a schema-bounded JSON object only.[^CLAIM-106] |
| Toggles/flags | Request-time LLM requires `llm.enabled`, runtime-only `ILAS_LLM_ENABLED`, and runtime-only `ILAS_COHERE_API_KEY`; the admin form remains secretless.[^CLAIM-069][^CLAIM-098] |
| Failure + observability | Failure paths fall back to deterministic clarify/current-intent behavior, log minimized error signatures, and expose vendor-neutral runtime-truth reachability fields instead of Google-specific diagnostics.[^CLAIM-048][^CLAIM-075] |
...
| Sentry status | Effective runtime Sentry wiring is present in sampled `local`/`dev`/`test`/`live`: safe runtime checks showed a client key, rendered `/assistant` pages exposed browser Sentry config with environment/release tags, fresh 2026-03-16 post-edit PHP probe runs emitted event IDs in all four sampled environments, a post-upload Pantheon `test` browser helper event returned `1e805c654fa8472ab0d7d66f3e9c2798` with scrubbed client context before send, and a browser exception proof returned `26abb98d151b45f0990bff8c91dec67a`. Account-side Sentry proof now confirms both browser and PHP events land in project `php`, ownership maps `assistant_name=aila` to Evan Curry, and live AILA alert rules exist. Operational usefulness remains `Unverified` overall only because GitHub workflow history is still failed-only and fresh browser JS stacks still lack resolved original coordinates. AFRP-12 proof taxonomy classifies Sentry probe at L1:Transport (transport reachability only); account-side proof (L4) requires manual Sentry UI verification with no programmatic lookup command available.[^CLAIM-083][^CLAIM-189][^CLAIM-190][^CLAIM-192][^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208][^CLAIM-266][^CLAIM-267] |
| Langfuse status | Effective runtime Langfuse enablement is present in sampled `local`/`dev`/`test`/`live`; direct probes still return HTTP `207`, and AFRP-03 proves fresh local direct, queued, and real request-path traces via Langfuse API lookups using fresh trace IDs. AFRP-17 upgrades the repo-side operator surface from raw counters to structured loss accounting: `ilas:langfuse-status` and `ilas:langfuse-probe --diagnose` now expose per-outcome counters, aggregate totals, `action_required`, alertable/informational loss totals, and outcome policy metadata, while local clean stale-item drills prove `discard_stale` now lands as an actionable queue-loss summary instead of only a log line. Fresh Pantheon reruns on 2026-03-19 still prove hosted `dev`/`test`/`live` direct and queued probe traces account-side, and one hosted `live` request trace resolves with the expected vector fields; the hosted command surface still predates AFRP-17 until deploy. AFRP-12 proof taxonomy still classifies direct probe at L3:PayloadAcceptance, queued probe at L2:QueueDrain, and account-side lookup at L4:AccountSide; AFRP-17 improves repo-side L5:Alertability coverage, but hosted L5/L6 parity remains deployment/documentation work.[^CLAIM-079][^CLAIM-080][^CLAIM-081][^CLAIM-082][^CLAIM-189][^CLAIM-190][^CLAIM-209][^CLAIM-210][^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-266][^CLAIM-267][^CLAIM-276][^CLAIM-277][^CLAIM-278] |
...
| SLO policy + alerts | `SloDefinitions` + `SloAlertService` define/enforce availability, latency, error-rate, cron freshness, and queue depth/age SLOs with cooldowned structured warning alerts from cron. AFRP-17 adds a separate queue-loss warning path for new alertable Langfuse loss outcomes (`drop_max_depth`, terminate enqueue failure, stale/invalid/misconfigured discards, and partial `207` rejections) without making historical queue-loss counters turn health checks sticky-red.[^CLAIM-084][^CLAIM-121][^CLAIM-277] |
...
| Langfuse queue behavior | Items are aged/validated, discarded when stale/disabled/misconfigured, and transient API failures suspend queue for retry. AFRP-17 extends queue-health tracking from depth/age-only into structured loss accounting: discard/drop outcomes now carry per-outcome queue-item/event totals, terminate-side enqueue failure is explicit, `send_partial_207` counts rejected events as partial loss, and health/metrics surfaces expose the same summary under `queue.export` while top-level queue health still keys only off backlog/staleness.[^CLAIM-082][^CLAIM-084][^CLAIM-276][^CLAIM-277][^CLAIM-278] |
...
| Config defaults vs active | Active config is now synced with install defaults: all blocks (`fallback_gate`, `safety_alerting`, `history_fallback`, `ab_testing`, `langfuse`, full LLM sub-keys) are present in active export. `conversation_logging.enabled` is currently `false` in both install and active export, while runtime invariants still force redaction + user notice whenever operators enable it. `ConfigCompletenessDriftTest` enforces ongoing parity.[^CLAIM-093][^CLAIM-094][^CLAIM-124] |
...
| Key/secrets management | `_ilas_get_secret()` reads Pantheon runtime secrets or env vars; runtime config overrides remain in place for Cohere/Langfuse/Pinecone/Sentry and Voyage reranking, while residual Gemini key wiring is limited to the proven Search API / vector path. The custom assistant request-time path no longer loads Vertex credentials from `settings.php`, and no exported assistant provider credential material remains in synced config.[^CLAIM-097][^CLAIM-098][^CLAIM-241] |
...
| Conversation logging | `conversation_logging.*` | `enabled=false`, retention `72h`, redaction true | `enabled=false`, retention `72h`, redaction true | Effective runtime is currently `enabled=false` in sampled environments; if operators enable it later, runtime forces `redact_pii=true`, `show_user_notice=true`, and caps retention at `720h` | DB logging remains opt-in and privacy-constrained by service invariants | [^CLAIM-047][^CLAIM-088][^CLAIM-093][^CLAIM-094][^CLAIM-119] |
| LLM master switch | `llm.enabled` | `false` | `false` in export; runtime-toggle controlled in hosted environments | Local runtime truth now reflects `settings.php` override channels; live no longer hard-forces the assistant LLM off in code, but remains opt-in via `ILAS_LLM_ENABLED` | Gated before any Cohere call and still defaults off in export | [^CLAIM-069][^CLAIM-094][^CLAIM-099][^CLAIM-119] |
| LLM provider/model | `llm.provider`, `llm.model` | `cohere`, `command-a-03-2025` | `cohere`, `command-a-03-2025` | Runtime truth reports the explicit config value and the source of the effective value | Deterministic assistant flow may invoke Cohere only for bounded intent classification and authorized diagnostics proof | [^CLAIM-073][^CLAIM-074][^CLAIM-093][^CLAIM-094][^CLAIM-119] |
| LLM credentials | `ILAS_COHERE_API_KEY`, `ILAS_LLM_ENABLED`; residual `ILAS_GEMINI_API_KEY` only where Search API AI still proves needed | No request-time provider secrets in install config | Runtime-only site setting for Cohere; no exported Vertex credential surface remains | `settings.php` injects Cohere via runtime-only site setting and keeps Gemini separate from the assistant path | Values redacted intentionally; Vertex runtime secret loading was removed from the custom assistant path | [^CLAIM-069][^CLAIM-098][^CLAIM-241] |
| LLM proof commands | `ilas:cohere-probe`, `ilas:runtime-diagnostics --probe-llm` | No probe state | Exact-output Cohere probe stored in sanitized runtime state when invoked | Runtime truth/diagnostics distinguish config readiness from last explicit generation reachability proof | Probes send only the harmless `ILAS_COHERE_PROBE_OK` check and never expose secrets | [^CLAIM-069][^CLAIM-098] |
| LLM generation params | `llm.max_tokens`, `llm.temperature` | `150`, `0.3` | `150`, `0.3` | Same request-time classification path | Structured JSON classification only; greeting variation and free-form summaries stay retired | [^CLAIM-072][^CLAIM-093][^CLAIM-094] |
| LLM retries/timeout | `llm.max_retries`; code timeout `5s`, connect timeout `2s` | `1` | `1` (synced) | Matches install default | Retryable HTTP codes include 429/5xx; sync retry delay capped at `<=250ms` | [^CLAIM-075][^CLAIM-093][^CLAIM-094][^CLAIM-124][^CLAIM-167] |
...
| Vector supplement | `vector_search.*` | Present, `enabled=false` | Present, `enabled=false` (synced) | Stored config remains `false`, but current runtime truth on 2026-03-18 shows effective `true` in `local` / `dev` / `test` and effective `false` in `live` because enablement is runtime-controlled | Schema coverage is complete, but exported YAML is storage-only and no longer authoritative runtime proof | [^CLAIM-061][^CLAIM-093][^CLAIM-094][^CLAIM-095][^CLAIM-096][^CLAIM-124] |
...
| Langfuse enablement | `langfuse.enabled` | `false` | `false` (synced) | Effective runtime is live-only by default: non-live remains disabled unless `ILAS_LANGFUSE_ENABLED=1`, and live auto-enables with credentials unless `ILAS_LANGFUSE_ENABLED=0` | Queue depth/age/timeout keys remain in config path; queue age thresholds are now 7200s, and queue health derives oldest age from actual queue rows where available | [^CLAIM-079][^CLAIM-082][^CLAIM-098][^CLAIM-189][^CLAIM-190][^CLAIM-210][^CLAIM-259][^CLAIM-260][^CLAIM-261] |
...
|---|---|---|---|---|---|
| Langfuse effective enablement | `config:get ilas_site_assistant.settings langfuse.enabled` | Sync storage reports `false` and does not show live-only/toggle runtime policy | `drush ilas:runtime-truth` | Runtime truth is authoritative: non-live should report effective `false` unless `ILAS_LANGFUSE_ENABLED=1`; live should report effective `true` when credentials exist and no kill switch is set | [^CLAIM-213][^CLAIM-214][^CLAIM-216] |
...
| Assistant controls beyond observability | `config:get ilas_site_assistant.settings ...` | Exported assistant YAML does not show runtime secret-backed values, live-only overrides, or service-level normalization for rate limits, conversation logging, retrieval URL health, LLM readiness, or Voyage readiness | `drush ilas:runtime-truth` | AFRP-02 expands the helper and admin report so those assistant-setting families are now exposed as sanitized stored-versus-effective runtime truth in local and Pantheon read-only checks |  |
...
now includes a broader "Runtime Truth Status" section that groups core assistant
controls, retrieval/runtime-only URL status, observability integrations, queue
health, and stored-vs-effective divergences without requiring Drush access. The
focused `drush ilas:langfuse-status` command remains useful for Langfuse-only
machine checks, but `drush ilas:runtime-truth` is the canonical assistant
settings runtime truth surface.

AFRP-02 (2026-03-18) expands the runtime-truth posture beyond the original
observability-heavy TOVR-08 helper. The same canonical JSON/report path now
distinguishes stored versus effective values for message/read/session limits,
conversation logging invariants, retrieval IDs and runtime-only LegalServer URL
health, LLM runtime readiness and cost-control config, Voyage runtime readiness,
and the previously covered Langfuse/Sentry/Pinecone/GA surfaces. The current
2026-03-18 baseline remains explicit: `local` / `dev` / `test` still show
stored `vector_search.enabled=false` versus effective `true`; `live` keeps
effective `vector_search.enabled=false`; Langfuse now remains stored `false`
everywhere, effective `false` on non-live unless explicitly toggled, and
effective `true` on `live` when credentials exist and no kill switch is set; `live`
still shows effective `google_tag_id=true`; and `diagnostics_token_present`
remains `false` in every sampled environment.
...
| Langfuse ingestion and queue-export success | `current trace proof complete / deployed operator surface pending` | AFRP-03 now proves fresh local direct, queued, and request-path traces account-side; current Pantheon `dev` / `test` / `live` direct and queued probes also resolve account-side; and one hosted `live` request trace now proves the TOVR-13 vector fields on the current release. | Deploy AFRP-03 so Pantheon also exposes the new `ilas:langfuse-lookup` and export-counter `ilas:langfuse-status` surfaces, then rerun hosted queue-loss drills from deployed code if full operator-visible parity is required. | [^CLAIM-209][^CLAIM-210][^CLAIM-248][^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-262] |
...
| Live Pinecone rollout quality acceptance | `runtime-toggle contract refreshed / hosted proof pending` | TOVR-12 still provides the non-live enablement proof, TOVR-13 still preserves the historical live blocker set, and TOVR-17 now updates the repo contract so `live` can be runtime-enabled without changing exported config or the live admin form. | Deploy the refreshed contract, set `ILAS_VECTOR_SEARCH_ENABLED=1`, clear caches, and capture `ilas:runtime-truth`, `ilas:vector-status`, smoke, and rollback proof together with the remaining prompt-quality and monitoring acceptance evidence. | [^CLAIM-232][^CLAIM-238][^CLAIM-246][^CLAIM-248][^CLAIM-250] |
...
| Retired Vertex assistant surface reintroduction | `active mitigation / narrowed residual risk` | The 2026-04-15 Cohere-first transition removed custom assistant Vertex secret loading and transport wiring; the remaining provider unknown is whether residual Gemini Search API/vector configuration can also be reduced further without breaking AI Search behavior. | Keep Vertex reintroduction blocked by contract tests and finish the prove-and-clean review on residual Gemini vector/Search API usage. | [^CLAIM-241][^CLAIM-248][^CLAIM-250] |
...
2. Contract fields are assembled by `assembleContractFields()` and injected
   across the current 200-response branches: safety exit, repeated-message
   escalation, OOS exit, policy violation exit, normal pipeline completion,
   and retrieval-unavailable degraded navigation. Error responses (4xx/5xx)
   are excluded and retain their minimal shape.[^CLAIM-134]
...
1. `tests/run-quality-gate.sh` is the mandatory module-level gate and now
   ...
   (Safety + OutOfScope), and conversation intent fixture checks. Those
   fixture checks validate classifier/resolver behavior only; public assistant
   conversation quality is covered by API-level functional and Promptfoo
   multi-turn tests.
...
1. **Credential availability**: Runtime evidence (`phase1-observability-gates.txt`)
   ...
   provisioned via `_ilas_get_secret()` on Pantheon. Langfuse credentials now
   inject into `ilas_site_assistant.settings.langfuse.*` only when live-only
   policy or `ILAS_LANGFUSE_ENABLED=1` enables export; Sentry DSN still wires to
   `raven.settings.client_key`.[^CLAIM-097][^CLAIM-098][^CLAIM-126]
...
### Re-Audit Remediation RAUD-03 Vertex Runtime Secret Disposition (2026-03-09; superseded 2026-04-15)

This dated addendum records re-audit remediation `RAUD-03` for findings `C2`
and `F-15`.

The 2026-04-15 Cohere-first transition supersedes the custom assistant Vertex
runtime path described below. Keep this section as historical context only; the
current assistant request-time path no longer loads or uses Vertex secrets.

1. At that time, the custom admin form no longer exposed an editable Vertex service-account
...
2. At that time, `settings.php` loaded `ILAS_VERTEX_SA_JSON` into the Drupal site setting
...
3. At that time, `LlmEnhancer` read the Vertex credential from the runtime site setting
...
1. Scope boundary closure is explicit and enforced: assistant channels remain
   ...
   provider wiring remains limited to the audited Cohere-first request-time
   path plus already-proven retrieval providers with no net-new channel or
   speculative provider expansion.[^CLAIM-073][^CLAIM-074][^CLAIM-158]
...
[^CLAIM-252]: [CLAIM-252](evidence-index.md#claim-252)
[^CLAIM-253]: [CLAIM-253](evidence-index.md#claim-253)
[^CLAIM-254]: [CLAIM-254](evidence-index.md#claim-254)
[^CLAIM-255]: [CLAIM-255](evidence-index.md#claim-255)
[^CLAIM-259]: [CLAIM-259](evidence-index.md#claim-259)
[^CLAIM-260]: [CLAIM-260](evidence-index.md#claim-260)
[^CLAIM-261]: [CLAIM-261](evidence-index.md#claim-261)
[^CLAIM-262]: [CLAIM-262](evidence-index.md#claim-262)
