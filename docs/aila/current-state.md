...
| Langfuse tracing/export | Install/export defaults remain disabled, but the 2026-03-16 safe runtime check still showed `langfuse.enabled=true` plus both keys present | Safe runtime check still showed `langfuse.enabled=true`, both keys present, and `langfuse.sample_rate=1` in `dev`/`test`/`live` effective config | TOVR-04 fixed the queued probe contract, aligned direct probe/worker behavior, and locally proved both direct trace `7e70bedf-c7f6-4e9a-9b4d-3467360b859f` and queued trace `ab0a7e4c-41d7-4c28-92c9-618e9c38ed9f` via Langfuse trace API lookups. Pantheon queued reruns still need to happen after deploy. | [^CLAIM-079][^CLAIM-082][^CLAIM-189][^CLAIM-190][^CLAIM-209][^CLAIM-210] |
| Sentry integration | Effective runtime config showed `raven.settings.client_key` present and `/assistant` rendered browser Sentry config with `browserEnabled=true` | Same runtime/browser result in `dev`/`test`/`live`; live hides the report dialog while keeping browser tracing/replay config | Fresh 2026-03-16 post-edit PHP probes emitted event IDs in all sampled environments, Pantheon `test` helper-path browser capture returned event ID `1e805c654fa8472ab0d7d66f3e9c2798` with scrubbed client context, a browser exception proof returned event ID `26abb98d151b45f0990bff8c91dec67a`, and Sentry verification now proves both browser and PHP events land in project slug `php` with `user.email`/`user.ip_address` null. Ownership plus permanent live AILA rules are now configured, while the remaining gaps are a successful GitHub workflow rerun and browser JS source-map usefulness proof. | [^CLAIM-083][^CLAIM-189][^CLAIM-190][^CLAIM-192][^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208] |
...
| Promptfoo harness | Repo-local npm scripts + provider remain in use, and the first-party `Quality Gate` workflow had recent completed runs on 2026-03-16 | Pantheon target can be used via URL env var; representative job logs plus TOVR-05 remediation now resolve the prior-versus-current split | Prior status: representative PRs ran real promptfoo evals in advisory mode while representative protected-branch pushes ran simulated config-parity mode in blocking status. Post-change status: PR/helper branches run real hosted deploy-profile checks in advisory mode, protected-branch pushes run real hosted deploy-profile checks in blocking mode, and synced `origin/master` deploys now run a local DDEV exact-code deploy-bound gate before the Pantheon push. Hosted GitHub checks are no longer treated as deploy proof for `origin/master`. | [^CLAIM-188][^CLAIM-200][^CLAIM-211] |
| Observability Release automation | Workflow and Sentry CLI script exist in repo | A 2026-03-16 manual run (`23164126480`) built theme assets successfully and failed at the Sentry upload step because the remote workflow checkout still used older `sentry-cli` syntax; the local repo now uses current `releases` + `sourcemaps upload` commands, and a write-capable local run successfully finalized Sentry releases `test_155` and `test_156` | Manual-only (`workflow_dispatch`) and still unproven in successful GitHub execution history | [^CLAIM-188][^CLAIM-192][^CLAIM-199][^CLAIM-206][^CLAIM-208] |
...
| Promptfoo gate behavior | Evaluation / release gating | `confirmed with hosted-rate-limit split` | Synthetic assistant eval gate and policy enforcement | GitHub Actions + operator-run local/DDEV/Pantheon targeting | TOVR-05 follow-up proves helper publish PRs and protected pushes both use real hosted blocking checks, ordinary PRs remain hosted advisory, hosted GitHub runs now use the smaller `promptfooconfig.hosted.yaml`, and synced `origin/master` deploys stay blocked by a local DDEV exact-code gate using `promptfooconfig.deploy.yaml` | `high` | Fresh post-change workflow-history proof for the next helper publish PR and next protected-branch push after the hosted-profile split | [^CLAIM-188][^CLAIM-200][^CLAIM-211] |
| Observability Release workflow | CI/CD observability | `configured but partially proven` | Manual Sentry release/source-map preparation | GitHub Actions `workflow_dispatch` | Workflow now builds theme assets with source maps and stages only deployable upload assets; the first recorded run on 2026-03-16 (`23164126480`) failed at older `sentry-cli` upload syntax before the local repo fix could be rerun, while local write-capable execution finalized `test_155` and `test_156` | `high` | Successful GitHub workflow run plus browser JS de-minification proof | [^CLAIM-188][^CLAIM-192][^CLAIM-206][^CLAIM-208] |
...
| Sentry release/source-map automation | Observability release | `confirmed partial` | Publish release metadata and theme source maps | GitHub Actions manual path | TOVR-03 hardens the workflow to build theme assets before upload, limits the upload bundle to deployable `css`/`js` assets, and updates the script to current `sentry-cli` commands; local write-capable runs finalized `test_155` and `test_156`, but the current bundle only exposes one actual sourcemap (`style.css.map`) and no successful GitHub rerun exists yet | `high` | Successful GitHub release run and original-source JS frame resolution | [^CLAIM-192][^CLAIM-206][^CLAIM-208] |
| Langfuse runtime export | Observability | `confirmed partial` | Queue-backed trace/span/event export | `local`, `dev`, `test`, `live` | Safe runtime booleans show enabled effective config; TOVR-04 now proves fresh local direct and queued traces in Langfuse API with privacy-safe payload summaries | `high` | Post-deploy queued rerun on Pantheon-hosted code | [^CLAIM-189][^CLAIM-190][^CLAIM-210] |
...
| Internal Drupal telemetry surfaces | Internal observability | `confirmed with private machine path` | Health/metrics/admin reporting, queue, cron, and SLO telemetry | Drupal runtime; health/metrics are now admin-or-machine-auth while reports/conversations remain Drupal-only | TOVR-07 preserved anonymous `403 access_denied`, added runtime-only machine auth for `/assistant/api/health` and `/assistant/api/metrics`, and kept analytics/no-answer/conversation storage metadata-only. Local positive-path execution is covered by new functional tests; deployed positive-path HTTP checks still need post-deploy proof where the diagnostics secret is web-scoped. | `medium` | Positive machine-auth runtime checks in deployed environments plus longer-run cron/queue observation | [^CLAIM-011][^CLAIM-020][^CLAIM-084][^CLAIM-212] |
...
TOVR-05 (2026-03-16) hardened release gating in two steps: first by replacing simulated protected-push proof with real hosted Promptfoo plus a local DDEV deploy-bound gate, and then by closing the helper-PR honesty gap exposed by runs `23176299781` and `23176706947`. Helper `publish/master-*` PRs and protected pushes now both run hosted blocking checks, `git:finish` downloads and inspects `gate-summary.txt` before merge, GitHub Actions use the smaller `promptfooconfig.hosted.yaml` profile to stay under the shared Pantheon `dev` hourly limit, and synced `origin/master` deploys still rely on the local DDEV exact-code `promptfooconfig.deploy.yaml` gate for deploy proof.[^CLAIM-200][^CLAIM-211]
...
TOVR-08 (2026-03-17) replaces the old ad hoc `VC-RUNTIME-*` truth path with a repo-owned `ilas:runtime-truth` Drush command backed by `RuntimeTruthSnapshotBuilder`, so override-prone Langfuse, Sentry, Pinecone, GA, and runtime site-setting facts can be retrieved as sanitized stored-versus-effective JSON instead of inferred from raw `config:get` or one-off `php:eval` snippets. Local post-edit verification passed, while Pantheon `dev`/`test`/`live` still report `Command "ilas:runtime-truth" is not defined` because those hosted environments are serving pre-TOVR-08 code; older `config:get`-based `VC-RUNTIME` evidence is now treated as storage-only and historical.[^CLAIM-213][^CLAIM-214][^CLAIM-215]
...
| Promptfoo + quality gate harness | The repo now has an active first-party `Quality Gate` workflow, and the TOVR-05 follow-up closes the helper-PR honesty gap: helper `publish/master-*` PRs and protected-branch pushes both run real hosted Promptfoo in blocking mode, ordinary feature PRs remain hosted advisory, and synced `origin/master` deploys still run the local DDEV exact-code deploy-bound gate before Pantheon push. Hosted GitHub checks now use the smaller `promptfooconfig.hosted.yaml` so the Pantheon `dev` hourly budget has headroom, while the local deploy proof remains the full `promptfooconfig.deploy.yaml` suite. The hosted GitHub check remains useful hosted-environment evidence, but it is not deploy proof for `origin/master`.[^CLAIM-086][^CLAIM-188][^CLAIM-200][^CLAIM-211] |
...
|---|---|---|---|---|---|
| Langfuse effective enablement | `config:get ilas_site_assistant.settings langfuse.enabled` | Sync storage still reports `false` even while `settings.php` makes effective runtime `true` | `drush ilas:runtime-truth` | Fresh local output now emits the divergence directly; Pantheon predeploy baselines still reproduce `false` versus `true` until the new command is deployed there | [^CLAIM-213][^CLAIM-215] |
| Sentry runtime wiring | `config:get raven.settings` | The sync file is absent even while runtime overrides populate `raven.settings` | `drush ilas:runtime-truth` plus `/assistant` HTML sample | Local post-edit helper shows Sentry runtime presence without printing DSNs; Pantheon hosted rerun is deployment-gated for the new helper | [^CLAIM-213][^CLAIM-215] |
| Pinecone key presence | Exported key entity only | Sync storage intentionally keeps the key blank while `settings.php` injects the runtime value | `drush ilas:runtime-truth` | Local helper now reports stored `false` versus effective `true` without exposing the key material | [^CLAIM-214][^CLAIM-215] |
| Browser-only truth (Sentry flags + live GA) | Server-side config inspection alone | Browser settings are only authoritative when rendered into `/assistant` HTML | `curl /assistant` companion marker sample | Local and Pantheon samples still show `ilasObservability`/Sentry browser markers, and `live` alone still renders `googletagmanager` plus `dataLayer` | [^CLAIM-190][^CLAIM-215] |

Older `config:get`-based `VC-RUNTIME` examples are now storage-only and
historical. The canonical runtime-truth path is `drush ilas:runtime-truth`
plus the `/assistant` browser marker sample; Pantheon post-change helper proof
remains deployment-gated until the new command reaches `dev` / `test` /
`live`.[^CLAIM-214][^CLAIM-215]
...
|---|---|---|---|---|
| Sentry operational usefulness beyond runtime wiring | `partially proven / still unverified` | Fresh 2026-03-16 post-edit PHP probes emitted event IDs in `local`, `dev`, `test`, and `live`; post-upload Pantheon `test` browser events `1e805c654fa8472ab0d7d66f3e9c2798` and `26abb98d151b45f0990bff8c91dec67a` were verified with scrubbed context and null user email/IP; ownership now maps `assistant_name=aila` to Evan Curry; permanent live AILA rules exist; temporary test rules proved backend/browser alert-route execution; and local write-capable release uploads finalized `test_155` and `test_156`. | Remaining blockers are now narrow: GitHub run `23164126480` is still the only workflow-history record and it failed before upload, and the fresh browser exception still had no resolved original source coordinates even after the `test_156` artifact-bundle upload. | [^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208] |
| Langfuse ingestion and queue-export success | `local proof complete / hosted rerun pending` | TOVR-04 reran local direct and queued probes after patching the probe/worker, verified the queued row stored top-level `batch`/`metadata`/`enqueued_at`, observed queue-worker partial-success notices instead of invalid-item discard, and resolved both fresh trace IDs through Langfuse's trace API. | Repeat direct and queued probes on Pantheon after the patched code is deployed there; earlier hosted direct proof remains valid, but hosted queued proof is still deployment-gated. | [^CLAIM-209][^CLAIM-210] |
| New Relic entity activity and browser-snippet value | `resolved — retired (TOVR-06)` | All AILA-owned NR wiring retired: browser snippet injection, deploy change tracking, browser hooks, DDEV scaffold, and related config/docs removed. CWV/RUM gap addressable via GA4 or future platform-level decision. | None — retirement resolves the unknown. Post-deploy: remove NR Pantheon runtime secrets. | [^CLAIM-203] |
| Promptfoo deploy-bound gate fidelity | `repo remediated / post-change hosted history pending` | Prior status was proven from workflow history: PR run `23165709608` logged a real promptfoo eval in advisory mode, while protected-branch push run `23166428930` logged simulated config-parity mode in blocking status. The next helper PR run `23176299781` and post-merge push run `23176706947` then exposed the hosted `429 rate_limited` honesty gap. Post-change repo wiring now switches helper `publish/master-*` PRs and protected pushes to hosted blocking runs with `promptfooconfig.hosted.yaml`, ordinary feature PRs to hosted advisory runs, and synced `origin/master` pushes to a local DDEV exact-code blocking gate. | Capture the next post-fix helper publish PR run, the next protected-branch workflow run, and the next clean `origin/master` deploy push to extend workflow-history proof beyond repo/script inspection. Hosted GitHub proof remains intentionally scoped to hosted checks, while deploy proof now comes from the local DDEV gate. | [^CLAIM-200][^CLAIM-211] |
