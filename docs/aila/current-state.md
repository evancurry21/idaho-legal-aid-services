...
5. Live vector search is hard-disabled in `settings.php` even if a runtime toggle is present.
...
| Langfuse tracing/export | Install/export defaults remain disabled, but the 2026-03-18/19 reruns still showed `langfuse.enabled=true`, both keys present, `langfuse.sample_rate=1`, and current local direct/queued/request-path traces resolving account-side through Langfuse lookups | Safe runtime reruns on 2026-03-18 still showed `langfuse.enabled=true`, both keys present, and `langfuse.sample_rate=1` in `dev`/`test`/`live` effective config; AFRP-03 then reran fresh hosted direct and queued probes on all three environments plus one hosted `live` real request and resolved those trace IDs account-side | TOVR-04 fixed the queued probe contract, TOVR-13 added vector trace metadata, and AFRP-03 closes the current hosted trace-proof gap. The remaining Langfuse deployment gap is operational ergonomics: Pantheon is still serving the pre-AFRP-03 `ilas:langfuse-status` surface without the new export counters or lookup command, and Langfuse's top-level trace `environment` still appears as `default` while runtime environment is preserved in metadata. | [^CLAIM-079][^CLAIM-082][^CLAIM-189][^CLAIM-190][^CLAIM-209][^CLAIM-210][^CLAIM-248][^CLAIM-259][^CLAIM-260][^CLAIM-261][^CLAIM-262] |
...
| LLM provider/model | `llm.provider`, `llm.model` | code-owned Cohere transport | `cohere`, `command-a-03-2025` | Runtime truth now reports Cohere as the only request-time assistant provider | Deterministic assistant flow may invoke Cohere only for bounded intent classification | [^CLAIM-073][^CLAIM-074][^CLAIM-093][^CLAIM-094][^CLAIM-119] |
...
| Langfuse enablement | `langfuse.enabled` | `false` | `false` (synced) | Effective runtime was `true` in sampled `local`/`dev`/`test`/`live` because `settings.php` injected keys/enablement | Queue depth/age/timeout keys remain in config path; AFRP-03 now proves current account-side direct, queued, and request-path traces on both local and current Pantheon releases, while the new operator-facing lookup/counter surfaces remain deployment-gated on Pantheon | [^CLAIM-079][^CLAIM-082][^CLAIM-098][^CLAIM-189][^CLAIM-190][^CLAIM-210][^CLAIM-259][^CLAIM-260][^CLAIM-261] |
...
effective `vector_search.enabled=false`; `local` / `dev` / `test` / `live`
still show stored `langfuse.enabled=false` versus effective `true`; `live`
