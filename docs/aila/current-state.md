...
| Langfuse status | Effective runtime Langfuse enablement is present in sampled `local`/`dev`/`test`/`live`; direct probes still return HTTP `207`, and AFRP-03 now proves fresh local direct, queued, and real request-path traces via Langfuse API lookups using fresh trace IDs. The queued probe stores top-level `batch`/`metadata`/`enqueued_at`, the worker surfaces `207` partial success explicitly, the exported payload remains metadata-only, and local `ilas:langfuse-status` now exposes explicit `drop_max_depth`, `discard_stale`, and related export outcomes instead of leaving queue loss implicit. Fresh Pantheon reruns on 2026-03-19 now prove hosted `dev`/`test`/`live` direct and queued probe traces account-side, and one hosted `live` request trace resolves with the expected vector fields; the hosted command surface still predates AFRP-03 until deploy.[^CLAIM-079][^CLAIM-080][^CLAIM-081][^CLAIM-082][^CLAIM-189][^CLAIM-190][^CLAIM-209][^CLAIM-210][^CLAIM-259][^CLAIM-260][^CLAIM-261] |
