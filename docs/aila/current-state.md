...
| Sentry integration | Effective runtime config showed `raven.settings.client_key` present and `/assistant` rendered browser Sentry config with `browserEnabled=true` | Same runtime/browser result in `dev`/`test`/`live`; live hides the report dialog while keeping browser tracing/replay config | Fresh 2026-03-16 post-edit PHP probes emitted event IDs in all sampled environments, and a post-edit Pantheon `test` browser synthetic error returned event ID `163b2e3129ae4ca884586aaaf75f9438` with scrubbed client context. Repo-side release automation now builds theme assets and uploads only deployable assets, but current alert-routing, project-slug verification, and Sentry-side release/source-map proof remain blocked by missing local `SENTRY_AUTH_TOKEN` and zero workflow run history. | [^CLAIM-083][^CLAIM-189][^CLAIM-190][^CLAIM-192][^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208] |
...
| Observability Release automation | Workflow and Sentry CLI script exist in repo | `gh run list` still returned no recorded `observability-release.yml` runs as of 2026-03-16, but TOVR-03 now builds theme assets before upload and stages only deployable browser assets for source-map publication | Manual-only (`workflow_dispatch`) and still unproven in execution history | [^CLAIM-188][^CLAIM-192][^CLAIM-199][^CLAIM-206][^CLAIM-208] |
...
| Observability Release workflow | CI/CD observability | `configured but not proven active` | Manual Sentry release/source-map preparation | GitHub Actions `workflow_dispatch` | Workflow now builds theme assets with source maps and stages only deployable upload assets, but `gh run list` still returned zero recorded runs on 2026-03-16 | `high` | Successful workflow run or Sentry release/source-map artifact | [^CLAIM-188][^CLAIM-192][^CLAIM-206][^CLAIM-208] |
...
TOVR-03 (2026-03-16) hardened the repo-side Sentry release/source-map path, added explicit browser payload-contract coverage, reran pre/post-edit PHP and browser probes, and kept the overall Sentry verification state at `Unverified` because account-side proof is still blocked.[^CLAIM-206][^CLAIM-207][^CLAIM-208]
...
| Sentry status | Effective runtime Sentry wiring is present in sampled `local`/`dev`/`test`/`live`: safe runtime checks showed a client key, rendered `/assistant` pages exposed browser Sentry config with environment/release tags, fresh 2026-03-16 post-edit PHP probe runs emitted event IDs in all four sampled environments, and a post-edit Pantheon `test` browser synthetic error returned event ID `163b2e3129ae4ca884586aaaf75f9438` with scrubbed client context before send. Operational usefulness remains unproven overall because current Sentry API/UI access, alert routing, project-slug verification, and release/source-map evidence are still missing.[^CLAIM-083][^CLAIM-189][^CLAIM-190][^CLAIM-192][^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208] |
...
|---|---|---|---|---|
| Sentry operational usefulness beyond runtime wiring | `unproven` | Fresh 2026-03-16 post-edit PHP probes emitted event IDs in `local`, `dev`, `test`, and `live`; a post-edit Pantheon `test` browser synthetic error returned event ID `163b2e3129ae4ca884586aaaf75f9438` with scrubbed client context; TOVR-03 also fixed the repo-side workflow so source-map upload no longer depends on tracked `.map` files or a `node_modules` sweep. | Current shell still has no `SENTRY_AUTH_TOKEN`, PHARD-01 alert/ownership sections still contain placeholders, `SENTRY_PROJECT_SLUG_BROWSER` still needs verification from Sentry, and `observability-release.yml` still has no recorded run history. Next step: set a local token, verify the real browser project slug, dispatch the fixed workflow, and record alert-rule plus source-map proof from Sentry itself. | [^CLAIM-201][^CLAIM-206][^CLAIM-207][^CLAIM-208] |
