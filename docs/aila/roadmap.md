...
1. Current Pinecone secret/runtime truth is now explicit: `local`, `dev`, `test`, and `live` all report Pinecone key presence in effective runtime, and `vector_search.enabled=false` in effective runtime across all four sampled environments. (Refs: current-state §1, §5; evidence-index CLAIM-216, CLAIM-218; runbook §3)
